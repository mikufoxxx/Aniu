...
export const api = {
  ...
  buildQuantDataset(payload: { symbols?: string[]; limit?: number; prefer_realtime?: boolean; lookback_days?: number }) {
    return request<QuantDatasetPayload>(`${API_PREFIX}/quant/dataset`, {
      method: 'POST',
      body: JSON.stringify(payload),
      timeoutMs: 60000,
    })
  },
  buildAIMarketContext(payload: { symbols?: string[]; limit?: number; lookback_days?: number }) {
    return request<AIMarketContextPayload>(`${API_PREFIX}/market/ai-context`, {
      method: 'POST',
      body: JSON.stringify(payload),
      timeoutMs: 60000,
    })
  },
  buildAIStockPicks(payload: { symbols?: string[]; limit?: number; lookback_days?: number; prefer_realtime?: boolean }) {
    return request<AIStockPicksPayload>(`${API_PREFIX}/ai/picks`, {
      method: 'POST',
      body: JSON.stringify(payload),
      timeoutMs: 60000,
    })
  ...
  analyzeStock(payload: { symbol: string; initial_cash?: number; model?: string; skill_id?: string; skill_ids?: string[] }) {
  ...
  getArenaAgentDashboard(agentId: string, section = 'morning', date?: string) {
